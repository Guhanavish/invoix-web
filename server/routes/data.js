...
const { withUserDb, hasUserDb, queryAll, queryGet } = require('../lib/db');

const router = express.Router();
router.use(requireAuth);

function guard(req, res, fn) {
  if (!hasUserDb(req.userId)) {
    return res.status(404).json({ success: false, error: 'No data synced yet' });
  }
  withUserDb(req.userId, fn).then((result) => {
    if (result === null) {
      res.status(404).json({ success: false, error: 'No data synced yet' });
    } else {
...
router.get('/ledger/balances', (req, res) => {
  guard(req, res, async (db) => {
...
router.get('/reports/gstr3b', (req, res) => {
  guard(req, res, async (db) => {
    const from = req.query.from || '1900-01-01';
    const to = req.query.to || '2999-12-31';
    const cid = Number(req.query.company_id) || 1;
